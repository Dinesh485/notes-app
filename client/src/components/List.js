        ...
        {searchResult.length <= 0 && searchStr.length ===0 ? list.map(item => {
          return (
            ...
            <motion.li  initial = {{opacity: 0, scale: 0}} animate ={{opacity: 1, scale:1, }} exit = {{opacity: 0, scale: 0}} onClick={() => { dispatch(openItem(item.id)) }} layout key={item.id} transition={{ duration: 0.3, type: 'tween' }} className='group cursor-pointer dark:bg-[#363636] dark:text-white text-black bg-white shadow-md  transition-colors duration-300 dark:shadow-none relative p-3 px-4  rounded-xl  overflow-hidden h-full flex  flex-col md:rounded-2xl xl:p-5 ' >
              ...
              <h2 className='text-xl w-10/12 md:w-full font-medium mb-1 xs:mb-2 xl:text-2xl overflow-hidden relative before:w-1/6  before:block before:right-0 before:h-full before:absolute before:bg-gradient-to-r before:from-transparent  before:to-white dark:before:to-[#363636] before:transition-colors before:duration-100'>{item.title}</h2>
              <p className='text-sm  w-10/12 md:w-3/4 md:h-[60%] overflow-hidden md:break-words  xl:text-md  font-light  relative  before:w-1/6 md:before:w-full md:before:h-10 before:block before:right-0 md:before:bottom-0 before:h-full before:absolute before:bg-gradient-to-r md:before:bg-gradient-to-b before:from-transparent  before:to-white dark:before:to-[#363636] before:transition-colors before:duration-100' >{item.content}</p>
